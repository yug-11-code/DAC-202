...
Brain Tumor Binary Segmentation - BRISC Dataset

Binary variant: all tumor classes (glioma, meningioma, pituitary) merged into class 1 (tumor).
"""
...
def _read_binary_mask(mask_path):
    """Read mask and convert to binary: 0=background, 1=tumor."""
    ...
    # Threshold: anything > 127 is tumor
    binary = (mask > 127).astype(np.uint8)
    return binary


def section3_mask_analysis(matched):
    sep("SECTION 3 * Mask Analysis (Binary: background vs tumor)")
    ...
    print(f"Processing {len(matched)} masks (converting to binary)...")
    ...
    return pixel_counts, weights, images_with, pure_bg_count


def _colorize_mask(mask):
    """Return RGB colorized mask (H,W,3) for binary mask."""
...
def section4_visual_sanity(matched):
    sep("SECTION 4 * Visual Sanity Check (Binary)")
    ...
    col_titles = ["Original Image", "Mask Overlay", "Binary Mask"]
    ...
    plt.suptitle("Visual Sanity Check - Binary Segmentation", fontsize=15, y=1.01)
    plt.tight_layout()
...
def section5_three_channel_preview(matched):
    ...
    col_titles = ["Ch1: Original (Gray)", "Ch2: CLAHE", "Ch3: Sobel Edges", "GT Mask (Binary)"]
    ...
    plt.suptitle("3-Channel Model Input Preview (Binary)", fontsize=14, y=1.01)
    plt.tight_layout()
...
def _assign_label(img_path, mask_path):
    """Binary: 1 if tumor present, 0 otherwise."""
    try:
        mask = _read_binary_mask(mask_path)
        if mask.any():
            return 1
...
def section6_split(matched):
    sep("SECTION 6 * Train / Val / Test Split  (80 / 10 / 10)")

    records = []
    for img_path, mask_path in matched:
        label = _assign_label(img_path, mask_path)
        records.append({
    ...
    print("\nTumor presence per split:")
    for split in ["train", "val", "test"]:
        subset = df[df.split == split]
        tumor_count = subset["has_tumor"].sum()
        total = len(subset)
        print(f"  {split:6s}: {tumor_count}/{total} images have tumor ({100*tumor_count/total:.1f}%)")
...
def section7_summary(matched, unmatched_imgs, unmatched_masks,
                     ...
                     images_with, pure_bg_count, df_splits):
...
| Image sizes (W*H -> count)                   |""")
    ...
    print(f"""+---------------------------------------------+
| Class Pixel Distribution (Binary)           |
|  {'Class':<12} {'Pixels':>12} {'%':>6} {'RMIF_w':>8}   |""")
    ...
    print(f"""+---------------------------------------------+
| Images containing tumor                     |
|  tumor     : {images_with.get(1, 0):<28}|
|  pure bg   : {pure_bg_count:<28}|""")
...
| Warnings                                    |""")
    ...
    ax.set_title("Class Pixel Distribution - Binary Segmentation")
    ax.set_yscale("log")
    plt.tight_layout()
...
def main():
    ...
    print("=" * 60)
    print("  Brain Tumor Binary Segmentation - Step 1: Data Exploration")
    ...
    pixel_counts, weights, images_with, pure_bg_count = \
    ...
    df_splits = section6_split(matched)
